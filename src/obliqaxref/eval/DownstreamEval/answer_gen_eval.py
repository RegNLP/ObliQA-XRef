...
- Subset test splits: ObliQA-XRef-{CORPUS}-{DPEL|SCHEMA}-ALL-test.jsonl (falls back to combined test filtered by method)
- IR run files for contexts: ObliQA-XRef-{CORPUS}-ALL/{bm25|e5|rrf|ce_rerank_union200}.trec
...
- Save per-subset × method to ObliQA-XRef_Out_Datasets/answer_gen_{corpus}_{subset}_{method}_test.json
...
from obliqaxref.eval.DownstreamEval import answer_eval as _answer_eval
...
def load_subset_items(corpus: str, subset: str, root: Path) -> list[dict[str, Any]]:
    ...
    # fallback: combined
    combined = root / f"ObliQA-XRef-{corpus.upper()}-ALL-test.jsonl"
...
def load_ir_run(
    ...
    """
    trec = root / f"ObliQA-XRef-{corpus.upper()}-ALL" / f"{method}.trec"
    if not trec.exists():
...
def discover_methods(corpus: str, root: Path) -> list[str]:
    ...
    dirp = root / f"ObliQA-XRef-{corpus.upper()}-ALL"
    if not dirp.exists():
        return []
    methods: list[str] = []
