...
def _load_final_dataset_files(corpus: str) -> dict[str, Path]:
    ...
    base = Path("ObliQA-XRef_Out_Datasets")
    return {
        "all": base / f"ObliQA-XRef-{corpus_tag}-ALL.jsonl",
        "train": base / f"ObliQA-XRef-{corpus_tag}-ALL-train.jsonl",
        "dev": base / f"ObliQA-XRef-{corpus_tag}-ALL-dev.jsonl",
        "test": base / f"ObliQA-XRef-{corpus_tag}-ALL-test.jsonl",
    }
...
def _load_final_per_method_files(corpus: str, method: str) -> dict[str, Path]:
    ...
    base = Path("ObliQA-XRef_Out_Datasets")
    return {
        "all": base / f"ObliQA-XRef-{corpus_tag}-{m}-ALL.jsonl",
        "train": base / f"ObliQA-XRef-{corpus_tag}-{m}-ALL-train.jsonl",
        "dev": base / f"ObliQA-XRef-{corpus_tag}-{m}-ALL-dev.jsonl",
        "test": base / f"ObliQA-XRef-{corpus_tag}-{m}-ALL-test.jsonl",
    }
...
def main(corpus: str, output_dir: Path = None) -> None:
    ...
    # Also save under ObliQA-XRef_Out_Datasets/DatasetStats/{corpus}
    dataset_stats_dir = Path("ObliQA-XRef_Out_Datasets") / "DatasetStats" / corpus
