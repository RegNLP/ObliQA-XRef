...
def finalize_dataset_main(
    ...
    corpus_tag = c.upper()
    full_path = out_root / f"ObliQA-XRef-{corpus_tag}-ALL.jsonl"
    _write_jsonl(records, full_path)

    splits = _split_dataset(records, seed=seed)
    for name, items in splits.items():
        _write_jsonl(items, out_root / f"ObliQA-XRef-{corpus_tag}-ALL-{name}.jsonl")
    ...
    for mtag, recs in method_groups.items():
        ...
        base = out_root / f"ObliQA-XRef-{corpus_tag}-{mtag_clean}-ALL.jsonl"
...
if __name__ == "__main__":
    ...
    ap.add_argument("--out_dir", default="ObliQA-XRef_Out_Datasets", help="Output directory")
