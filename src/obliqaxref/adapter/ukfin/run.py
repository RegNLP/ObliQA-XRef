# src/obliqaxref/adapter/ukfin/run.py
...
from obliqaxref.adapter.ukfin.discover import discover_pra_allowlist
from obliqaxref.adapter.ukfin.download import download_allowlist
from obliqaxref.adapter.ukfin.types import UkFinAdapterConfig, UkFinCorpusSource
...
def run(cfg: UkFinAdapterConfig, out: Path | None = None) -> Path:
    ...
    stats_json = processed_dir / "obliqaxref_stats.raw.json"
    ...
    if _stage_enabled(cfg, "corpus"):
        if _is_nonempty_file(corpus_jsonl):
            ...
            from obliqaxref.adapter.ukfin.corpus import generate_corpus
    ...
    if _stage_enabled(cfg, "crossref"):
        if _is_nonempty_file(crossref_csv):
            ...
            from obliqaxref.adapter.ukfin.crossref import generate_crossrefs
    ...
    if _stage_enabled(cfg, "clean"):
        ...
        from obliqaxref.adapter.ukfin.clean_crossref import clean_crossrefs
