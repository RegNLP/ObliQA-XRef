...
from obliqaxref.adapter.run import run
...
def run_adapter(
    ...
    if corpus_type == "ukfin":
        from obliqaxref.adapter.ukfin.config import load_ukfin_config

        cfg = load_ukfin_config(str(config))
    elif corpus_type == "adgm":
        from obliqaxref.adapter.adgm.config import load_adgm_config
